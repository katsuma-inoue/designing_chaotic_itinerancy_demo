...
class ESN(object):
    ...
    def reset_rls(self, mu=1.0, alpha=1.0, select=None,
        ...
        self.w_pre = {}
        self.P = {}
        for _post in post_node:
            non_zeros = self.w_net[_post].nonzero()[0]
            selected_id = np.isin(non_zeros, pre_node)
            self.w_pre[_post] = non_zeros[selected_id]
            self.P[_post] = np.eye(len(self.w_pre[_post]))
            self.P[_post] *= 1 / self.alpha
